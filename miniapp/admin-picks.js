(() => {
  ...
  if (!document.getElementById("yycj-pick-css")) {
    ...
    st.id = "yycj-pick-css";
    st.textContent = `
.pick-head{display:flex;align-items:center;justify-content:space-between;margin:4px 0 10px;}
.pick-head h4{margin:0;font-size:14px;letter-spacing:.02em;}
.pick-count{font-size:12px;opacity:.6;}
.pick-now{display:grid;grid-template-columns:72px 1fr auto;gap:12px;align-items:center;padding:10px;border:1px solid #2a3352;border-radius:16px;margin:0 0 10px;background:linear-gradient(180deg,#171e33,#13192b);}
.admin-thumb{width:72px;height:72px;object-fit:cover;border-radius:12px;background:#0d1326;display:block;}
.admin-thumb.ph{background:#1a2140;}
.pick-now .meta{min-width:0;}
.pick-now .meta strong{display:block;font-size:15px;line-height:1.3;}
.pick-now .meta .muted{margin-top:4px;font-size:12px;line-height:1.4;}
.pick-now .pick-actions{display:flex;flex-direction:column;gap:6px;}
.pick-now .btn{white-space:nowrap;}
.pick-item{display:grid;grid-template-columns:56px 1fr auto;gap:10px;align-items:center;padding:10px;border:1px solid #2a3352;border-radius:14px;margin:8px 0;background:#151b2f;}
.pick-item .hrs{width:100%;margin:0;grid-column:1 / -1;}
#pane-pin .ops-card,#pane-carousel .ops-card{margin-top:8px;}
