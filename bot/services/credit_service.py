...
def _is_admin_uid(user_id: int) -> bool:
    try:
        ...
        return get_settings().is_admin(int(user_id))
    except Exception:
        return False
...
def _user_to_dict(u: User) -> Dict[str, Any]:
    return {
        ...
        "is_shadowed": False if _is_admin_uid(u.user_id) else u.is_shadowed,
...
def _apply_shadow_rules(user: User, reason: str, *, delta: int = 0) -> None:
    if _is_admin_uid(getattr(user, "user_id", 0)):
...
async def ensure_user(user_id: int, username: str | None = None, full_name: str | None = None) -> Dict[str, Any]:
    async with session_scope() as s:
        ...
        if user:
            ...
            if _is_admin_uid(user_id) and user.is_shadowed:
                user.is_shadowed = False
                user.shadow_days = 0
                user.shadow_reason = None
        ...
        user = User(user_id=user_id, username=username, full_name=full_name, lanhua_score=DEFAULT_SCORE, tier=tier_from_score(DEFAULT_SCORE).value, is_shadowed=False, shadow_days=0)
        s.add(user)
        await s.flush()
        return _user_to_dict(user)
...
async def settle_lanhua(user_id: int, delta: int, action: str, reason: str, related_id: str | None = None) -> Dict[str, Any]:
    async with session_scope() as s:
        res = await s.execute(select(User).where(User.user_id == user_id))
        user = res.scalar_one_or_none()
        if not user:
            user = User(user_id=user_id, lanhua_score=DEFAULT_SCORE, tier=tier_from_score(DEFAULT_SCORE).value, is_shadowed=False, shadow_days=0)
            s.add(user)
            await s.flush()
        old_score = int(user.lanhua_score)
        new_score = max(0, min(1000, old_score + int(delta)))
        user.lanhua_score = new_score
        user.tier = tier_from_score(new_score).value
        user.updated_at = datetime.utcnow()
        if delta > 0:
        ...
        s.add(CreditHistory(user_id=user_id, action=action, delta=int(delta), reason=reason, related_id=related_id))
        await s.flush()
        return _user_to_dict(user)

async def history(user_id: int, limit: int = 20) -> List[Dict[str, Any]]:
    async with session_scope() as s:
        res = await s.execute(select(CreditHistory).where(CreditHistory.user_id == user_id).order_by(CreditHistory.time.desc()).limit(limit))
        return [{"user_id": r.user_id, "time": r.time, "action": r.action, "delta": r.delta, "reason": r.reason, "related_id": r.related_id} for r in res.scalars().all()]

async def tick_shadow_daily() -> int:
    async with session_scope() as s:
        res = await s.execute(select(User).where(or_(User.is_shadowed.is_(True), User.lanhua_score < 200)))
        users = list(res.scalars().all())
        count = 0
        for u in users:
            if _is_admin_uid(u.user_id):
                if u.is_shadowed:
                    u.is_shadowed = False
                    u.shadow_days = 0
                ...
                continue
            if u.is_shadowed:
                u.shadow_days = max(0, int(u.shadow_days or 0) - 1)
            if int(u.lanhua_score) < 200:
                ...
                s.add(CreditHistory(user_id=u.user_id, action="daily_recovery", delta=DAILY_RECOVERY_POINTS, reason="日恢复（低分缓慢回升）", related_id=None))
...
def format_credit_card(user: Dict[str, Any]) -> str:
    ...
    lines = [f"🌸 <b>兰花令 · {tier}</b>", f"信用分：<code>{score}</code> / 1000", f"累计获得：{user.get('total_earned', 0)}  ·  累计扣除：{user.get('total_deducted', 0)}"]
...
def compute_heuristic_q(message_count: int, media_count: int, duration_minutes: float = 0.0, reply_balance: float | None = None) -> int:
    ...
    return max(0, min(100, int(round(min(70, 2 * i + 5 * m) + min(15.0, d / 8.0) + min(15.0, 15.0 * bal)))))

def calc_session_delta(message_count: int, media_count: int, duration_minutes: float, has_praise: bool, reported: bool, brush_factor: float = 0.0, reply_balance: float | None = None, quality_score: int | None = None) -> int:
    i = min(int(message_count or 0), 80)
    d = min(float(duration_minutes or 0), 120.0)
    q = max(0, min(100, int(quality_score))) if quality_score is not None else compute_heuristic_q(message_count=i, media_count=int(media_count or 0), duration_minutes=d, reply_balance=reply_balance)
    raw = (8 + 0.18 * i + 0.12 * d + 0.25 * q + (12 if has_praise else 0)) * (1 - max(0.0, min(0.9, float(brush_factor or 0.0)))) - (40 if reported else 0)
    return max(SESSION_DELTA_MIN, min(SESSION_DELTA_MAX, int(round(raw))))
